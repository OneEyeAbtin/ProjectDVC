...
describe('ipc app:init', () => {
  it('returns full boot state', () => {
    ...
    expect(boot.personaGroups).not.toHaveProperty('💕 Dere Types')
    expect(boot.personaGroups).toHaveProperty('💝 Relationship', expect.arrayContaining(['Friend']))
    expect(boot.greetings.Friend).toContain('{name}')
    expect(boot.greetings.Friend).toContain('[EMOTION:')
    expect(boot.greetings.Gothic).toContain('[EMOTION: smirk]')
    expect(typeof boot.greetings.Vampire).toBe('string')
    expect(Object.keys(boot.greetings).length).toBe(21)
    expect(boot.personaTransforms.Friend).toContain('[EMOTION:')
    expect(Object.keys(boot.personaTransforms).length).toBe(21)
    expect(typeof boot.personaDescriptions.Friend).toBe('string')
    expect(Object.keys(boot.personaDescriptions).length).toBe(21)
